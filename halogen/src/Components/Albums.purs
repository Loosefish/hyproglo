...
render state =
    ...
    (links : H.hr_ : map albumRow albums)
    ]
    where
    name = (\(Artist { name }) -> name) state.artist
    albums = state.albums
    header = H.h4 [toClass "page-header clickable"] $ 
        (H.text $ name <> nbsp) : if state.busy then [fa "circle-o-notch fa-spin"] else []
